...
class post extends user {
    ...
    public function affichage(int $debut, int $nbelem): array{
        return $this->request("SELECT publication.id_publication, publication.message, user.id_user, user.pseudo, publication.date FROM `publication` INNER JOIN user ON publication.id_user = user.id_user ORDER BY date DESC LIMIT $debut, $nbelem;",[])->fetchAll();
    }
